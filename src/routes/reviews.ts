...
import {getReviews, getReview, getRandom, getLanding, getRandomHoliday, test, fillStreaming} from '../utils/data_handler';
        ...
        if(result) {
            ...
            if(user_id){
                ...
                .then(async({user_watch, user_seen}:any) => {
                    const d = (await getLanding()).map((li: Review[]) =>(
                        idInList(li, user_watch, user_seen)
                    ))
...
router.get('/search', handleQuery, async(req: Request, res: Response) => {
    ...
    const reviews: (Review[] | number)[] = await getReviews(sort as string, ratings as Ratings, parseInt(req.headers.skip as string), parseInt(req.headers.page as string),
...
router.get('/random', async(req: Request, res: Response) => {
    ...
    const results: (Review | Streaming[])[] = await getRandom(genres as string, subgenres as string, decades as string, 
...
router.get('/lists/:username', [auth, handleQuery], async(req: Request, res: Response) => {
    const {sort, ratings, directors, genres, subgenres, studiocompanies, universes, subuniverses, 
        characters, sportholidays, years, decades, providers, awards, runtime, type, ratingrange} = req.headers;
    ...
    .then(async({user_watch, user_seen}:any) => {
        ...
        if(!(type === 'watch'? user_watch : user_seen).length) throw('NO LISTS')
            ...
            const reviews: (Review[] | number)[] = await getReviews(sort as string, ratings as Ratings,  parseInt(req.headers.skip as string), 
                parseInt(req.headers.page as string), directors as string, genres as string, subgenres as string, 
                studiocompanies as string, universes as string, subuniverses as string, characters as string, sportholidays as string, 
                years as string, decades as string, providers as string, awards as string, 
                runtime as string, ratingrange as string, req.body.query, (type === 'watch'? user_watch : user_seen).join('@'));
